...
export function openKVDb <T = JsonValue>(options?: DBOptions): KVDb<T> {
  ...
  const fnDelete: (key: string) => void =
    (key) => pqDelete.execute([String(key)]);
  ...
  const fnEntriesJson: () => IterableIterator<Entry<string, T>> =
    function* () {
  ...
  const fnGet: (key: string) => string | null = (key) => {
    ...
    const value =
      (iter.next().value as [value: string] | undefined)?.[0] ?? null;
    return value;
  };
  ...
  const fnSet: (key: string, value: string) => void =
    (key, value) => pqSet.execute([String(key), String(value)]);

  const fnSetJson: (key: string, value: T) => void =
    (key, value) => pqSet.execute([String(key), JSON.stringify(value)]);
